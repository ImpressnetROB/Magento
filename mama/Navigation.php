...
class EDPA_MegaNavigation_Block_Navigation extends Infortis_UltraMegamenu_Block_Navigation{
	...
	//PRIVATE VARIABLES
	//*********************************
	private $exception = array('Baby Monitor Sale',
				   'Stairgates Sale', 
				   'Bathroom Sale',
				   'Feeding Sale',
				   'High Chairs and Booster Seats Sale',
				   'Cots and Sleeping Sale',
				   'Car Seats Sale',
				   'Locks & Latches Sale',
				   'Toys Sale',
				   'Toddler Travel Sale'
	...
	private function renderFullNavigation($category_level_0,$base_url,$urlfunction){
		...
		foreach($category_level_0 as $index_level_0 => $data_level_0){
			...
			if($level_0_name == 'Brands'){
				...
				$output .= "<li {$class}>";
				$output .= "<a href='" . Mage::getBaseUrl().$data_level_0->getRequestPath(). "' class='level-top red'><span>{$level_0_name}</span>{$caret}</a>";
	...
	private function getNavigationName($nav_name){
                ...
                switch ($nav_name) {
                    ...
                    case 'Pushchairs & Strollers':
                        $display = 'Strollers';
                        break;											case 'New Year Sale':                        $display = 'Sale';                        break;
